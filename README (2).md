...
> Конференція **IMA 2026** · Сумський державний університет, гр. ІН-31/1
...
- Запити — **тільки через `PreparedStatement`** (конкатенація SQL з введенням користувача заборонена)
...
## 🖥️ Функціонал (обов'язковий)

Через графічний інтерфейс застосунок забезпечує повний **CRUD** для таблиць `movie`, `viewer`, `rating`:
...
- Мінімум **5 змістовних комітів** від кожного учасника
